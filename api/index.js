...
bot.on('message',async(ctx)=>{
  ...
  if(state.stage==='AWAITING_WALLET_AMOUNT'){const amount=Number(String(ctx.message.text||'').trim().replace(/[۰-۹]/g,d=>'۰۱۲۳۴۵۶۷۸۹'.indexOf(d)));const required=Number(state.requiredAmount||0);if(!Number.isInteger(amount)||amount<1)return ctx.reply('❌ مبلغ نامعتبر است. یک عدد مثبت به تومان وارد کنید.');if(amount<Math.max(0,required-Number(state.balance||0)))return ctx.reply(`❌ این مبلغ کافی نیست. حداقل ${Math.max(0,required-Number(state.balance||0)).toLocaleString('en-US')} تومان لازم است.`);await createWalletTopup(ctx,amount,state.orderId);return;}
...
bot.command('wallet',async(ctx)=>{await persistUser(ctx);const balance=await wallet.getBalance(ctx.from.id);return ctx.reply(`💰 <b>کیف پول من</b>\n\nموجودی: <b>${balance.toLocaleString('en-US')} تومان</b>\n\nبرای شارژ کیف پول، مبلغ موردنظر را ارسال کنید.`,{parse_mode:'HTML'});});
