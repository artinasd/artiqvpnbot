...
function getConfiguredServices(config = {}) {
  ...
  return SERVICES.map((service) => {
    ...
    const enabled = legacyEnabled === true && (!hasSavedObject || saved.enabled === false)
      ? true
      : hasSavedObject && saved.enabled !== undefined
        ? saved.enabled !== false
        : legacyEnabled !== undefined
          ? legacyEnabled !== false
          : true;
    const testEnabled = hasSavedObject && saved.testEnabled !== undefined ? saved.testEnabled !== false : true;
    const purchaseEnabled = hasSavedObject && saved.purchaseEnabled !== undefined ? saved.purchaseEnabled !== false : true;
    return {
      ...service,
      ...(hasSavedObject ? saved : {}),
      id: service.id,
      name: hasSavedObject && saved.name ? saved.name : service.name,
      emoji: hasSavedObject && saved.emoji ? saved.emoji : service.emoji,
      description: hasSavedObject && saved.description ? saved.description : service.description,
      provider: hasSavedObject && saved.provider ? saved.provider : service.provider,
      order: Number(hasSavedObject ? (saved.order ?? service.order) : service.order),
      enabled,
      testEnabled,
      purchaseEnabled,
    };
