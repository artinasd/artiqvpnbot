...
class PasarGuardError extends Error {
  constructor(message, status, operation, transient = false) {
    super(message); this.name = 'PasarGuardError'; this.status = status; this.operation = operation; this.transient = transient;
  }
}

function configured() { return Boolean(BASE && (token || (USERNAME && PASSWORD))); }
function safeError(body) {
  if (!body) return 'PasarGuard request failed';
  if (typeof body === 'string') return body.slice(0, 300);
  return String(body.detail || body.message || body.error || 'PasarGuard request failed').slice(0, 300);
}

async function authenticate() {
  if (!BASE || !USERNAME || !PASSWORD) throw new PasarGuardError('PasarGuard credentials are not configured', 0, 'auth');
  const form = new URLSearchParams({ username: USERNAME, password: PASSWORD });
  const res = await fetch(`${BASE}/api/admin/token`, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: form });
  const text = await res.text();
  let body; try { body = JSON.parse(text); } catch { body = text; }
  if (!res.ok || !body?.access_token) throw new PasarGuardError(safeError(body), res.status, 'auth', res.status >= 500 || res.status === 429);
  token = body.access_token;
  tokenExpiresAt = Date.now() + 20 * 60 * 1000;
  return token;
}

async function request(method, path, body, operation, attempt = 0) {
  if (!BASE) throw new PasarGuardError('PASARGUARD_BASE_URL is not configured', 0, operation);
  if (!token || Date.now() >= tokenExpiresAt) await authenticate();
  const headers = { Authorization: `Bearer ${token}`, Accept: 'application/json' };
  if (body !== undefined) headers['Content-Type'] = 'application/json';
  let response;
  try {
    response = await fetch(`${BASE}${path}`, { method, headers, body: body === undefined ? undefined : JSON.stringify(body), signal: AbortSignal.timeout(15000) });
  } catch (err) {
    if (attempt < 2) { await new Promise(r => setTimeout(r, 400 * (2 ** attempt))); return request(method, path, body, operation, attempt + 1); }
    throw new PasarGuardError('network failure', 0, operation, true);
  }
  const text = await response.text();
  let data; try { data = text ? JSON.parse(text) : null; } catch { data = text; }
  if (response.status === 401 && attempt === 0 && !ACCESS_TOKEN) { token = null; tokenExpiresAt = 0; return request(method, path, body, operation, 1); }
  if (!response.ok) {
    const transient = response.status === 429 || response.status >= 500;
    throw new PasarGuardError(safeError(data), response.status, operation, transient);
  }
  return data;
}
...
async function disableUserById(id, disabled = true) { return request('PUT', `/api/user/by-id/${encodeURIComponent(id)}/disabled`, { is_disabled: disabled }, 'disable_user'); }
