...
function normalize(config) { const out = merge(DEFAULTS, config); out.services.directEnabled = false; delete out.services.tunnelEnabled; delete out.services.gamingEnabled; out.resellerTelegramIds = Array.from(new Set((Array.isArray(out.resellerTelegramIds) ? out.resellerTelegramIds : []).map(value => String(value).trim()).filter(value => /^\d+$/.test(value)))); for (const id of ['tunnel', 'gaming']) { const service = out.services[id]; service.name = String(service.name || DEFAULTS.services[id].name).trim() || DEFAULTS.services[id].name; service.emoji = String(service.emoji || DEFAULTS.services[id].emoji).trim() || DEFAULTS.services[id].emoji; service.description = String(service.description ?? DEFAULTS.services[id].description).trim(); service.order = Number.isFinite(Number(service.order)) ? Number(service.order) : DEFAULTS.services[id].order; service.enabled = service.enabled !== false; service.testEnabled = service.testEnabled !== false; service.purchaseEnabled = service.purchaseEnabled !== false; } out.limits.testLimitPerDay = Math.max(0, Number(out.limits.testLimitPerDay) || 0); out.limits.testTrafficBytes = Math.max(0, Number(out.limits.testTrafficBytes) || 0); out.limits.testDurationDays = Math.max(1, Number(out.limits.testDurationDays) || 1); out.limits.testHwidLimit = Math.max(0, Number(out.limits.testHwidLimit) || 0); out.bot.maintenanceMode = Boolean(out.bot.maintenanceMode); out.bot.qrBackground = String(out.bot.qrBackground || 'bg.png').split(/[\\/]/).pop() || 'bg.png'; return out; }
