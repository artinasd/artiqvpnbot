(function () {
  ...
  function render() {
    if (!current.length) {
      $('plans').innerHTML = '<p class="muted">پلنی وجود ندارد.</p>';
      return;
    }
    $('plans').innerHTML = '';
    current.forEach(function (p) {
      ...
      meta.className = 'meta';
      meta.textContent = 'ID: ' + p.id + ' · ' + Number(p.price).toLocaleString('en-US') + ' ' + (p.currency || '') + ' · ' + p.durationDays + ' روز · ' + (p.type === 'unlimited' ? 'نامحدود' : bytes(p.trafficBytes)) + ' · HWID ' + p.hwidLimit;
      ...
      editBtn.onclick = function () { edit(p.id); };
      actions.appendChild(editBtn);
      if (p.active) {
        var removeBtn = document.createElement('button');
        removeBtn.className = 'danger';
        removeBtn.textContent = '🗑️ غیرفعال';
        removeBtn.onclick = function () { removePlan(p.id); };
  ...
  function edit(id) {
    var p = current.find(function (x) { return x.id === id; });
    if (!p) return;
  ...
  async function savePlan() {
    try {
      var body = {
        ...
        trafficBytes: Number($('trafficBytes').value || 0),
        hwidLimit: Number($('hwidLimit').value || 0),
        type: $('type').value,
  ...
  async function removePlan(id) {
    ...
    try {
      await api('/api/admin', { method: 'POST', body: JSON.stringify({ action: 'remove', id: id }) });
      await loadPlans();
    } catch (e) {
      $('msg').className = 'error';
      $('msg').textContent = '❌ ' + e.message;
