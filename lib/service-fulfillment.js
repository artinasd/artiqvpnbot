...
function getFulfillment(serviceId, config = {}) {
  ...
  if (service.provider === 'pasarguard') {
    return { ok: true, code: 'PASARGUARD', service, provider: service.provider };
  }

  if (service.provider === 'pasarguard-panel2') {
    return { ok: true, code: 'PASARGUARD_PANEL2', service, provider: service.provider };
  }
