...
async function getConfig() { return merge(DEFAULTS, await storage.get(KEY)); }
async function saveConfig(patch) { const next = merge(DEFAULTS, await storage.get(KEY)); const merged = merge(next, patch || {}); await storage.set(KEY, merged); return merged; }
