...
bot.command('sendto',async(ctx)=>{if(!isAdmin(ctx))return;const raw=String(ctx.message?.text||'').replace(/^\\/sendto\\s*/i,'').trim();const parts=raw.split(/\\s+/);const id=parts.shift();const messageText=parts.join(' ').trim();if(!id||!/^[0-9]+$/.test(id)||!messageText)return ctx.reply('❌ استفاده: /sendto <telegram_id> <متن پیام>');try{await bot.telegram.sendMessage(id,messageText);return ctx.reply(\`✅ پیام برای کاربر \${id} ارسال شد.\`);}catch(error){log('ADMIN_SENDTO_FAILED',{telegram_user_id:id,error:error?.message||String(error)});return ctx.reply('❌ ارسال پیام ناموفق بود.');}});

bot.command('broadcast',async(ctx)=>{if(!isAdmin(ctx))return;const messageText=ctx.message.text.replace(/^\\/broadcast\\s*/,'').trim();if(!messageText)return ctx.reply('❌ استفاده: /broadcast متن پیام');const users=await storage.listActiveBotUsers(10000);let success=0;let failed=0;for(const user of users){const id=String(user.telegramUserId||'');if(!/^[0-9]+$/.test(id))continue;try{await bot.telegram.sendMessage(id,messageText);success++;}catch(error){failed++;if(/blocked|chat not found|user is deactivated|bot was blocked/i.test(String(error?.description||error?.message||''))){await storage.markUserBlocked(Number(id),'BROADCAST_FAILED');}}await new Promise(resolve=>setTimeout(resolve,50));}await ctx.reply(\`✅ ارسال پایان یافت. موفق: \${success} | ناموفق: \${failed} | هدف: \${users.length}\`);});
