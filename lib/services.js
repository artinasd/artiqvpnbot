...
function serviceButtons(prefix, config) {
  return getConfiguredServices(config).map((service) => [
    { text: `${service.label || `${service.emoji || ''} ${service.name}`} — ${service.description || ''}`, callback_data: `${prefix}${service.id}` },
  ]);
}

module.exports = { SERVICES, getService, getConfiguredServices, serviceButtons };
