...
const botLocal = new Telegraf(BOT_TOKEN);
...
botLocal.hears('🎁 دریافت اکانت تست', async (ctx) => {
    ...
    // Notify Admin
    await botLocal.telegram.sendMessage(ADMIN_ID, `⚠️ *درخواست اکانت تست جدید*\n\nکاربر: ${ctx.from.first_name}\nآیدی: ${username}\nشناسه: \`${userId}\``, {
        parse_mode: 'Markdown',
...
botLocal.hears('🛒 خرید اشتراک', async (ctx) => {
...
botLocal.on('callback_query', async (ctx) => {
    ...
    if (data.startsWith('reject_buy_')) {
        ...
        await ctx.answerCbQuery();
        await botLocal.telegram.sendMessage(targetUserId, '❌ رسید پرداختی شما توسط مدیریت تایید نشد. اگر فکر می‌کنید اشتباهی رخ داده است، لطفاً با پشتیبانی تماس بگیرید.');
        ...
        if (state.stage === 'AWAITING_RECEIPT') {
            if (ctx.message.photo || ctx.message.document) {
                ...
                if (ctx.message.photo) {
                    ...
                    await botLocal.telegram.sendPhoto(ADMIN_ID, photoId, { caption: adminCaption, parse_mode: 'Markdown', ...adminButtons });
                } else {
                    await botLocal.telegram.sendDocument(ADMIN_ID, ctx.message.document.file_id, { caption: adminCaption, parse_mode: 'Markdown', ...adminButtons });
                }
...
botLocal.launch().then(() => {
    console.log('Telegram Bot successfully initiated and polling updates...');
});

process.once('SIGINT', () => { botLocal.stop('SIGINT'); server.close(); });
process.once('SIGTERM', () => { botLocal.stop('SIGTERM'); server.close(); });