...
// Main menu buttons are rendered as inline buttons directly under the welcome message.
// Telegram does not expose button colors for bots, so the premium/glassy look is achieved
// through compact emoji-led labels and a clean 2x2 + full-width grid.
    ...
    return {
      resize() {
        return Markup.inlineKeyboard([
          [Markup.button.callback(labels[0], 'main_test'), Markup.button.callback(labels[1], 'main_buy')],
          [Markup.button.callback(labels[2], 'main_wallet'), Markup.button.callback(labels[3], 'main_account')],
          [Markup.button.callback(labels[4], 'main_support')],
        ]);
      },
    };
  };
  ...
  Telegraf.prototype.on = function patchedOn(updateTypes, ...middlewares) {
    ...
    const wrapped = async (ctx, next) => {
      ...
      if (data === 'main_account') {
        ...
        if (!subscriptions.length) return ctx.reply(await getMessage('accountNoSubscription'));
      ...
      if (data === 'main_support') {
        ...
        return ctx.reply(await getMessage('support', { support_username: supportUsername }));
      }
