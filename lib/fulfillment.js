...
async function fulfillOrder(orderIdValue, telegram) {
  ...
  try {
    ...
    // Persist the username BEFORE remote creation so a timeout after creation can be recovered.
    ...
    await updateOrder(orderIdValue, { fulfillmentStatus: 'FAILED_RETRYABLE', failureReason: error instanceof PasarGuardError ? error.message : String(error.message || error) }).catch(() => {});
    const failedOrder = await getOrder(orderIdValue).catch(() => null);
    if (failedOrder?.telegramUserId) await setState('user', failedOrder.telegramUserId, { stage: 'AWAITING_RECEIPT_RETRY', orderId: orderIdValue });
