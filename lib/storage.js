...
async function saveUser(user) {
  ...
  // A successful test fulfillment sets testUsed=true. Count those successful
  // test subscriptions by Tehran calendar day so the limit resets at midnight
  // Iran time. Failed test attempts do not increment the counter.
...
async function getUser(telegramUserId) {
  ...
  return { ...user, testUsed: count >= 2, testUsageDate: today, testUsageCount: count };
}
