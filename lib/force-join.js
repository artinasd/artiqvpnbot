...
function getConfig() {
  return require('./bot-config').getConfig();
}
...
function shouldBypass(ctx) {
  ...
  if (String(process.env.ADMIN_ID || '') === String(ctx.from.id)) return true;
  const data = String(ctx.callbackQuery?.data || '');
  return data === 'main_home' || data === 'force_join_channels' || data === 'force_join_check';
}
...
// account-ui.js is loaded before the bot instance is created. Patch Telegraf's
// use() once so the gate is inserted before the bot's first middleware without
// changing Telegraf globally for every subsequent use() call.
if (!Telegraf.prototype.__artiqForceJoinPatched) {
  ...
  Telegraf.prototype.use = function patchedUse(...middlewares) {
    ...
    return nativeUse.call(this, ...middlewares);
  };

  Telegraf.prototype.__artiqForceJoinPatched = true;
}

module.exports = { normalizeForceJoin, membershipStatus, promptKeyboard, gateMiddleware };
