...
async function resolveUsername(order) {
  if (order.generatedPasarguardUsername) return order.generatedPasarguardUsername;
...
async function fulfillOrder(orderIdValue, telegram) {
  ...
  try {
    ...
    if (!['RECEIPT_SUBMITTED', 'PROVISIONING', 'PASARGUARD_USER_CREATED', 'SUBSCRIPTION_RETRIEVED'].includes(order.fulfillmentStatus)) {
      throw new Error('ORDER_NOT_ELIGIBLE');
    }

    await updateOrder(orderIdValue, { fulfillmentStatus: 'PROVISIONING', failureReason: null });
    ...
    let username = await resolveUsername(order);
    let pgUser = null;
    ...
    if (order.pasarguardUserId) {
      ...
      for (let attempt = 0; attempt < 10; attempt++) {
        ...
        if (existing) {
          ...
          if (String(existingNote).includes(`Order: ${orderIdValue}`)) {
            pgUser = existing;
            break;
          }
...
async function renewOrder(order, telegram) {
  const lockName = `renew:${order.telegramUserId}`;
  if (!(await acquireLock(lockName, 120))) return { locked: true };
  try {
    const user = await getUser(order.telegramUserId);
    if (!user?.currentPasarguardUserId) throw new Error('NO_EXISTING_PASARGUARD_ACCOUNT');
    const current = await getUserById(user.currentPasarguardUserId);
    ...
    await updateUserById(user.currentPasarguardUserId, {
      status: 'active',
