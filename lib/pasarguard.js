...
const REQUEST_TIMEOUT_MS = Number(process.env.PASARGUARD_TIMEOUT_MS || 7000);
...
function normalizeUserSubscription(user) {
  ...
  if (user.subscription_url) {
    return { ...user, subscription_url: normalizeSubscriptionUrl(user.subscription_url) };
  }
  return user;
}
...
async function request(path, { method = 'GET', body, operation = path, retry = true } = {}) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    safeLog('PASARGUARD_REQUEST', { operation, method, path, attempt: attempt + 1 });
    try {
      ...
      safeLog('PASARGUARD_RESPONSE', { operation, method, path, status: response.status, elapsed_ms: Date.now() - started });
      ...
      if (error instanceof PasarGuardError) {
        safeLog('PASARGUARD_ERROR', { operation, status: error.status, elapsed_ms: Date.now() - started, error: error.message, details: error.details || null });
        throw error;
      }
      const transient = isTransient(0, error);
      safeLog('PASARGUARD_ERROR', { operation, status: 0, elapsed_ms: Date.now() - started, error: error?.name === 'AbortError' ? 'TIMEOUT' : 'NETWORK_ERROR' });
      if (transient && attempt < maxAttempts - 1) {
        await new Promise((resolve) => setTimeout(resolve, 250 * (2 ** attempt)));
        continue;
      }
...
async function getGroups() {
  ...
  safeLog('PASARGUARD_GROUPS', { count: groups.length });
  return groups;
}

async function findUserByUsername(username) {
  try {
    return normalizeUserSubscription(await request(`/api/user/${encodeURIComponent(username)}`, { operation: 'find_user' }));
  } catch (error) {
...
async function createUser({ username, trafficBytes, expire, hwidLimit, note }) {
  ...
  return normalizeUserSubscription(user);
}

async function getUserById(userId) {
  return normalizeUserSubscription(await request(`/api/user/by-id/${encodeURIComponent(userId)}`, { operation: 'get_user_by_id' }));
}
