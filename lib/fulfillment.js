...
async function deliverSubscription(telegram, order, text, subscriptionUrl) {
  ...
  try {
    ...
    log('TELEGRAM_DELIVERY_SUCCESS', { order_id: order.orderId, telegram_user_id: order.telegramUserId, mode: 'rich' });
    return true;
  } catch (error) {
    log('TELEGRAM_DELIVERY_ERROR', { order_id: order.orderId, telegram_user_id: order.telegramUserId, mode: 'rich', error: error?.message || String(error) });
    try {
  ...
  try {
    ...
    if (order.renewal) {
      ...
      const delivered = await deliverSubscription(
        telegram,
        order,
        `🎉 <b>اشتراک شما تمدید شد!</b>\n\n╭──────────────\n📦 <b>سرویس:</b> ${escapeHtml(order.planName)}\n👤 <b>نام اشتراک:</b> <code>${escapeHtml(order.generatedPasarguardUsername)}</code>\n⏳ <b>اعتبار جدید:</b> ${escapeHtml(updated.expire || expire)}\n╰──────────────\n\n🔗 <b>لینک اشتراک:</b>\n<code>${escapeHtml(updated.subscription_url)}</code>\n\n📋 برای کپی سریع لینک، دکمه زیر را بزنید 👇`,
    ...
    const delivered = await deliverSubscription(
      telegram,
      order,
      `🎉 <b>اشتراک شما با موفقیت ساخته شد!</b>\n\n╭──────────────\n📦 <b>سرویس:</b> ${escapeHtml(order.planName)}\n📊 <b>حجم:</b> ${order.trafficLimitBytes === 0 ? 'نامحدود ♾️' : formatBytes(order.trafficLimitBytes)}\n⏳ <b>اعتبار:</b> ${order.durationDays} روز\n👤 <b>نام اشتراک:</b> <code>${escapeHtml(order.generatedPasarguardUsername)}</code>\n╰──────────────\n\n🔗 <b>لینک اشتراک:</b>\n<code>${escapeHtml(subscriptionUrl)}</code>\n\n📋 برای کپی سریع لینک، دکمه زیر را بزنید 👇`,
      subscriptionUrl
    );
    await updateOrder(orderIdValue, delivered ? { deliveryStatus: 'DELIVERED' } : { deliveryStatus: 'FAILED', failureReason: 'TELEGRAM_DELIVERY_FAILED' });
