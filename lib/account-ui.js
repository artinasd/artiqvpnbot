...
// The main-menu implementation temporarily used colored marker emojis. Telegram
// does not support button background colors, so normalize those markers back to
// the original button emojis while keeping the inline-button layout intact.
if (!Markup.__artiqOriginalMenuEmojiPatched) {
  ...
  Markup.button.callback = function restoreOriginalMenuEmoji(text, ...args) {
    ...
    value = value.replace(/^🟢\s*/, '🎁 ')
      ...
      .replace(/^🟡\s*/, '💰 ')
      .replace(/^⚪\s*/, '🎯 ');
    return nativeCallback.call(this, value, ...args);
  };
  Markup.__artiqOriginalMenuEmojiPatched = true;
}

function homeButton() {
  return { text: '🏠 منوی اصلی', callback_data: 'main_home' };
}
